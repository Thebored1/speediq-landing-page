...
export function DesktopNavLight() {
  ...
  return (
    <nav 
      className="flex h-full items-center divide-x divide-neutral-200 relative"
    ...
    >
      {MENUS.map((menu, idx) => (
        ...
        >
          <button className={`flex h-full items-center px-6 gap-2 bg-transparent text-sm font-medium transition-colors ${activeIdx === idx ? 'text-black bg-neutral-100' : 'text-black hover:bg-neutral-50'}`}>
            {menu.label}
      ...
      <Link 
        href="/pricing"
        className="flex h-full items-center px-6 text-sm font-medium text-neutral-700 hover:text-black hover:bg-neutral-100 transition-colors"
      ...
      >
        <div 
          className="w-72 bg-white border border-neutral-200 shadow-xl overflow-hidden mt-0 relative transition-all duration-150 ease-out" 
              ...
              >
                {menu.items.map((item) => (
                  <Link 
                    key={item.name}
                    href={item.href} 
                    className="p-3 text-sm flex flex-col gap-1 hover:bg-neutral-100 rounded-sm transition-all duration-200 group/item"
                  >
